...
- **公钥加密算法的 IND-CPA 安全性定义**：任意概率多项式时间敌手在 CPA 安全模型中攻破 IND 安全目标的**优势**（advantage）是可忽略的，也即
    ...
    - **等价定义**：
        ...
        =& \left| \Pr(\mathrm{output} = 0 | b = 0) \cdot \Pr(b=0) + \Pr(\mathrm{output} = 1 | b = 1) \cdot \Pr(b=1) - \frac{1}{2} \right| \\
        =& \frac{1}{2} \left| \Pr(\mathrm{output} = 0 | b = 0) + \Pr(\mathrm{output} = 1 | b = 1) - 1 \right| \\
        =& \frac{1}{2} \left| \Pr(\mathrm{output} = 0 | b = 0) - \Pr(\mathrm{output} = 0 | b = 1) \right| \\
        =& \frac{1}{2} \left| \Pr(\mathrm{output} = 1 | b = 0) - \Pr(\mathrm{output} = 1 | b = 1) \right|
        \end{aligned}
        $$
    ...
    - **证明**：以 SKH-PA 安全性为例，采用反证法（归约）
        ...
        - **证明前提错误**：构造一个概率多项式时间敌手 $B$，在 CPA 安全模型中攻破 IND 安全目标。
            ...
            - $B$ 的优势：
                ...
                \begin{aligned}
                &\left| \Pr(\mathrm{output} = b) - \frac{1}{2} \right| \\
                =& \left| \Pr(\mathrm{output} = b | SK' = SK) \cdot \Pr(SK' = SK) + \Pr(\mathrm{output} = b | SK' \neq SK) \cdot \Pr(SK' \neq SK) - \frac{1}{2} \right| \\
