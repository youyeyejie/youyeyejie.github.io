...
- **Thm 1.19**: There are infinitely many primes of the form $4n+3$.
    ...
    - Suppose there are only finitely many primes of the form $4n+3$, denoted by $p_1=4n_1+3, p_2=4n_2+3, \cdots, p_k=4n_k+3$,
        - **Case 1**: $k$ is odd, then $\prod_{i=1}^k p_i+4=\prod_{i=1}^k (4n_i+3)+4=4m+3$ for some $m\in \mathbb{Z}$ and it is a composite number, thus $\exists p_i$ s.t. $p_i\mid (\prod_{i=1}^k p_i+4)$, thus $p_i\mid 4$, contradiction since $p_i>3$.
        - **Case 2**: $k$ is even, then $\prod_{i=1}^k p_i+2=\prod_{i=1}^k (4n_i+3)+2=4m+3$ for some $m\in \mathbb{Z}$ and it is a composite number, thus $\exists p_i$ s.t. $p_i\mid (\prod_{i=1}^k p_i+2)$, thus $p_i\mid 2$, contradiction since $p_i>3$.
