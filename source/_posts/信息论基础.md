...
index_img: /img/cover/nis2337.webp
date: 2025-04-23 01:36:32
update: 2025-04-23 01:36:32
---
...
## 通信系统模型
![通信系统模型](信息论基础/image.webp)

# 第二章 信源与信息熵

## 2.1 信源的分类及数学模型
![信源分类](信息论基础/image-1.webp)
...
- **马尔可夫链的稳定(稳态分布)**
    ...
    - **例题**：
        - ![例题](信息论基础/image-104.webp)
        - ![例题](信息论基础/image-105.webp)
        - ![例题](信息论基础/image-106.webp)
...
- 例题：![例题](信息论基础/image-110.webp)

### 互信息
- 未收到消息时，信源 $X$ 的不确定度为 $H(X)$，收到消息 $Y$ 后关于 $x_i$ 的不确定度为 $H(X|Y)$。![图例](信息论基础/image-111.webp)
- **定义**：
...
- **平均互信息的推导**
    ...
    - $I(X;y_j)$ 在 $Y$ 集合的**概率加权统计平均值，即平均互信息** $I(X;Y)$ 为
        ...
        \end{align*}
        $$ 
- **例题**：![例题](信息论基础/image-112.webp)

- **性质**：
    - $I(X;Y)=I(Y;X)=H(X)+H(Y)-H(X,Y)$
        - ![H(X，Y)](信息论基础/image-2.webp)
...
- **凸函数(Convex)定义**
    ...
    - ![图例](信息论基础/image-7.webp)
- **凹函数(Concave)定义**
    - 如果 $-f$ 为凸函数，则称函数 $f$ 是凹的
    - 如果函数总是位于任何一条弦的上面，则该函数是凹的
    - ![图例](信息论基础/image-113.webp)
...
- **三变量马尔可夫链的信息图**
    - 对于马尔可夫链 $X \to Y \to Z$ ，其信息图及相关性质如下：
        - ![三变量马尔可夫链的信息图](信息论基础/image-5.webp)
    - **相关等式与性质**：
...
- 例题：![例题](信息论基础/image-116.webp)

### 费诺不等式
#### 定义
- ![定义](信息论基础/image-6.webp)
...
- **定义信息熵$H(\vec{X_{}})$为**：
    $$
    \begin{align*}
    H(\vec{X_{}}) &= -\sum_{i = 1}^{n^L}p(\vec{x_{i}})\log p(\vec{x_{i}})\\
...
-   $$
    ...
    $$ 则 $$
    \begin{align*}
    H(\vec{X_{}})&=-\sum_{i_1 = 1}^{n}\sum_{i_2 = 1}^{n}\cdots\sum_{i_L = 1}^{n}p(x_{i1})p(x_{i2})\cdots p(x_{iL})[\log p(x_{i1})+\log p(x_{i2})\\
...
- 若信源是平稳的，即$H(X_1)=H(X_2)=\cdots=H(X_L)=H(X)$，则$$H(\vec{X_{}}) = L H(X)$$ 
    **用 $H_L(\vec{X_{}})$ 表示长度为 $L$ 的序列平均每个符号的熵**，则
    $$H_L(\vec{X_{}})=\frac{1}{L}H(\vec{X_{}}) = H(X)$$

- 例题：![例题](信息论基础/image-119.webp)
...
3. $H_L(\vec{X_{}})$ 是 $L$ 的单调非增函数
    - **证明**：$$
        ...
        \therefore (L - 1)H_L(\vec{X_{}})&\leq (L - 1)H_{L - 1}(\vec{X_{}})\\
...
4. 当 $L \to \infty$ 时，定义$H_{\infty}(\vec{X_{}})$为**极限熵**，有
    ...
    - **证明**：
        $$
        \begin{align*}
        H_{L + k}(\vec{X_{}})&=\frac{1}{L + k}[H(X_1,X_2,\cdots,X_{L - 1})+H(X_L|X_1X_2\cdots X_{L - 1})+\\
...
- **m阶马尔可夫信源（齐次）的推导**
    ...
    $$ $$
    \begin{align*}
    H_{\infty}(\vec{X_{}})&=\lim_{L \to \infty} H(X_L|X_1,X_2,\cdots,X_{L - 1}) \\
...
2. **命题2**：
    - **半正定矩阵的特征值是非负的**。
    - **证明**：
        令 $K$ 表示一个半正定矩阵，$\vec{u_{}}$ 是矩阵 $K$ 对应于特征值 $\lambda$ 的特征向量，即 $$K\vec{u_{}}=\lambda\vec{u_{}}$$
...
#### 无噪无损信道：n = m，X、Y一一对应
- 输入输出关系：![输入输出关系](信息论基础/image-20.webp)
- 转移概率矩阵$P = \{p(y_j|x_i)\} = 
    \begin{bmatrix}
    1 & 0 & \cdots \\
    0 & 1 & \cdots \\
...
#### 无噪有损信道：n > m，多个X对应一个Y
- 输入输出关系：![输入输出关系](信息论基础/image-21.webp)
- 多个输入对应一个输出，即$n > m$
- **噪声熵**:
...
#### 有噪无损信道：n < m，一个X对应多个Y
- 输入输出关系：![输入输出关系](信息论基础/image-22.webp)
- 信道噪声使一个输入对应多个输出，$n < m$
- **噪声熵**：
...
- 例题：![例题](信息论基础/image-98.webp)

-  **一般离散无记忆模k加性噪声信道**
    - 信道模型: $Y = X\oplus Z\bmod k$，其中 $X,Y,Z\in\{0,1,\cdots,k - 1\}$
    - 图例:![图例](信息论基础/image-23.webp)
    - **加性噪声**，有 $p(y|x)=p(z)$
        - **条件熵**:
            $$
            \begin{align*}
...
- 注意：**最佳输入分布不唯一！**
- 例题：![例题](信息论基础/image-33.webp)

## 3.3 离散序列信道及其容量
### 信道模型与符号定义
- ![信道模型](信息论基础/image-24.webp)
...
- **互信息与信道容量**:
    $$
    \begin{align*}
    I(\vec{X_{}};\vec{Y_{}})&=H(\vec{X_{}}) - H(\vec{X_{}}|\vec{Y_{}})=\sum p(\vec{X_{}},\vec{Y_{}})\log\frac{p(\vec{X_{}}|\vec{Y_{}})}{p(\vec{X_{}})}\\
    &=H(\vec{Y_{}}) - H(\vec{Y_{}}|\vec{X_{}})=\sum p(\vec{X_{}},\vec{Y_{}})\log\frac{p(\vec{Y_{}}|\vec{X_{}})}{p(\vec{Y_{}})}
    \end{align*}
    $$
    - **信道无记忆时**：
    ...
    - **当输入矢量$\vec{X_{}}$独立且信道无记忆时**，上述两个性质统一取等号，此时**信道容量**：
        $$
        \begin{align*}
        C_L&=\max_{p(x)}I(\vec{X_{}};\vec{Y_{}})=\max_{p(x)}\sum_{l = 1}^{L}I(X_l;Y_l)\\
...
- 示例：![示例](信息论基础/image-101.webp)

### 独立并联信道
- 图例: ![图例](信息论基础/image-25.webp)
...
2. **$L$ 个高斯噪声均值为0，方差不同且为 $\sigma_l^2$ 时**，若输入信号的总平均功率受限，即
    ...
    - **例题**：![例题](信息论基础/image-88.webp)

### 限时限频限功率加性高斯白噪声信道
#### 波形信道，限时 $t_B$，限频 $\omega$
- **信道模型**：
    - ![信道模型](信息论基础/image-30.webp)
- **互信息**：
    $$
    \begin{align*}
    I(x(t); y(t)) &= \lim_{L \to \infty} I(\vec{X_{}}; \vec{Y_{}}) \\
    &= \lim_{L \to \infty} [H_c(\vec{X_{}}) - H_c(\vec{X_{}}|\vec{Y_{}})] \\
    &= \lim_{L \to \infty} [H_c(\vec{Y_{}}) - H_c(\vec{Y_{}}|\vec{X_{}})] \quad \text{bit/波形}
    \end{align*}
    $$
- **单位时间内的信息传输率** $R_t$ 为：
    $$
...
### 信息率失真函数 $R(D)$
- 将信源编码器看作信道：![假想信道](信息论基础/image-35.webp)
- 编码的目的：
    - 使传输率 $R$ 尽量小
...
3. **$R(D)$函数的定义域和值域**
    ...
    - $R(D)$函数的最大值域为$$[0, H(X)]$$
    - ![函数图像](信息论基础/image-37.webp)

- 例题：
    - ![例题](信息论基础/image-90.webp)
    - ![例题](信息论基础/image-91.webp)

#### $R(D)$函数的下凸性和连续性
- ![函数图像](信息论基础/image-38.webp)
- **下凸性**：
...
1. **例题1**：
    ...
    **证明**：$R(D)=\begin{cases}H_b(p)-H_b(D), & 0\leq D\leq p \\ 0, & D > p\end{cases}$，其中$H_b(p)=H(p,1 - p)$，$H_b(D)=H(D,1 - D)$
    ...
    - **证明下界可达**：
        构造反向BSC信道如图：![BSC信道](信息论基础/image-41.webp)
...
- **例题**：
    - 例题1：![例题](信息论基础/image-93.webp)
    - 例题2：![例题](信息论基础/image-94.webp)
...
- **香农编码图示**
    ...
    - ![香农编码图示](信息论基础/image-45.webp)

- **例题**：![例题](信息论基础/image-58.webp)
...
- **累积概率递推公式**
    - **二元序列**：
        ...
        当$A = \{0,1\}$，即**二元信源**时：$P_{0}=0$；$P_{1}=p_{0}$![](信息论基础/image-48.webp)
        $$
        \begin{align*}
...
- **示例**：有四个符号$a$，$b$，$c$，$d$构成简单序列$S = abda$，各符号及其对应概率如下表![](信息论基础/image-75.webp)
...
- 页面数据结构![页面数据结构](信息论基础/image-76.webp)
- 结尾码码表![结尾码码表](信息论基础/image-77.webp)
- 组合基干码码表![组合基干码码表](信息论基础/image-78.webp)
- 例题：![例题](信息论基础/image-97.webp)
...
- **典型集$A_{\varepsilon}$**
    - **定义**
        $$
        \begin{align*}
        &A_{\varepsilon}=\left\{\vec{x_{}}:\left|\frac{I(\vec{x_{}})}{L}-H(X)\right| \leq \varepsilon\right\}\\
        &A_{\varepsilon}^c=\left\{\vec{x_{}}:\left|\frac{I(\vec{x_{}})}{L}-H(X)\right| > \varepsilon\right\}\\
    ...
    - **性质**
        ...
        3. 典型集中元素的个数：
            ...
            - 证明：
                $$
                \begin{align*}
                1&=\sum_{\vec{x_{}} \in X^L} p(\vec{x_{}}) \geq \sum_{\vec{x_{}} \in A_{\varepsilon}} p(\vec{x_{}})\\
                &\geq \sum_{\vec{x_{}} \in A_{\varepsilon}} 2^{-L(H(X)+\varepsilon)}=|A_{\varepsilon}|2^{-L(H(X)+\varepsilon)}\\
        ...
        4. 典型集中元素的个数：
            ...
            - 证明：
                $$
                \begin{align*}
                1-\varepsilon &\leq P_r(A_{\varepsilon}) = \sum_{\vec{x_{}} \in A_{\varepsilon}} p(\vec{x_{}})\\
                & \leq \sum_{\vec{x_{}} \in A_{\varepsilon}} 2^{-L(H(X)-\varepsilon)}=|A_{\varepsilon}|2^{-L(H(X)-\varepsilon)}\\
...
- **证明**
    ...
    - 反之，若
        ...
        $$
        \begin{align*}
        P_r(只出现A_{\varepsilon}中的m^{K_L}个元素) &\leq m^{K_L} \cdot \max_{\vec{x_{}} \in A_{\varepsilon}} p(\vec{x_{}}) = m^{K_L}2^{-L(H(X) - \varepsilon)}\\
...
- **证明**
    ...
    $$
    \begin{align*}
    \frac{L H_L(\vec{X_{}})}{\log m} &\leq \overline{k}_L < \frac{L H_L(\vec{X_{}})}{\log m} + 1\\
    H_L(\vec{X_{}}) &\leq \frac{\overline{k}_L}{L} \log m < H_L(\vec{X_{}}) + \frac{\log m}{L}
    \end{align*}
    $$
...
### 信道编码定理
- 信道编码定理研究的问题：![](信息论基础/image-61.webp)
...
- **系统码**
    - **码字结构**：码字$c$包含信息位（$k$位）和校验位（$n - k$位）![](信息论基础/image-67.webp)
    - **特点**：
...
- **特点**：可检测长度小于$r + 1$ bits的所有突发错误。![](信息论基础/image-84.webp)
- **CRC示例**：![](信息论基础/image-85.webp)
- **常用CRC版本**：![](信息论基础/image-86.webp)
- **CRC有效性**：![](信息论基础/image-87.webp)
...
## 译码规则和译码错误概率
- 已知信道转移错误概率 $p = 0.9$，转移情况如下：![](信息论基础/image-53.webp)
    - 若规定：
        - $Y = 0 \to \hat{X} = 0$
        - $Y = 1 \to \hat{X} = 1$
...
### 错误概率与编码方法
- 转移概率如图：![](信息论基础/image-55.webp)
- 采用**简单重复编码**，$k = 1$ ，$n = 3$ ，则$0\rightarrow000$（$\vec{C_{1}}$），$1\rightarrow111$（$\vec{C_{2}}$）
- 已知信道转移概率 $p = 0.01$ ，$\overline{p}=1-p=0.99$ ，转移概率矩阵：![](信息论基础/image-56.webp)
    <!-- $$P =\begin{bmatrix}
