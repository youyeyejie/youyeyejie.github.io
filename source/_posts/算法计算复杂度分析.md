...
- 关系
    ...
    \exists~q \in \mathrm{NPC}, q \in \mathrm{P} &\Rightarrow \forall p \in \mathrm{NP}, p \in \mathrm{P} \quad \Rightarrow \mathrm{P} = \mathrm{NP}
    \end{cases}
    $$

    ![](算法计算复杂度分析/image-84.webp)
- The relationship most theoretical computer scientists believe: $P \neq NP$（大多数理论计算机科学家相信的关系：$P \neq NP$）
    ![](算法计算复杂度分析/image-67.webp)
...
- 复杂度分析
    ...
    - Recurrence tree: $T(n) = 2 T\left(\frac{n}{2}\right) + c_{2} n$
        ![Recurrence tree](算法计算复杂度分析/image-4.webp)
        $$
...
- 示例：$T(n) = 8T\left(\frac{n}{2}\right) + c_2 n = \Theta (n^{3})$
    ![Example 2](算法计算复杂度分析/image-8.webp)
...
- 示例：$T(n) = 2T\left(\frac{n}{2}\right) + c_2 n = \Theta (n \log n)$
    ![Merge sort](算法计算复杂度分析/image-9.webp)
...
- 示例：$T(n) = 3T\left(\frac{n}{4}\right) + c_2 n^{2} = \Theta (n^{2})$
    ![Example 1](算法计算复杂度分析/image-10.webp)
...
- 示例：
    ![](算法计算复杂度分析/image-14.webp)

### 最优二叉搜索树 (Optimal binary search trees)
#### 问题描述
![](算法计算复杂度分析/image-15.webp)
...
- The expected cost of a search is
    ...
    $$
- 示例
    ![非最优解](算法计算复杂度分析/image-16.webp)
    ![最优解](算法计算复杂度分析/image-17.webp)
    ![Cut-and-paste](算法计算复杂度分析/image-18.webp)
...
- Let $S$ be an optimal solution to subproblem $(C, i)$
    ![](算法计算复杂度分析/image-25.webp)

- 基本思想
    ![](算法计算复杂度分析/image-2.webp)
...
![Processes and communication](算法计算复杂度分析/image-42.webp)
...
![](算法计算复杂度分析/image-44.webp)

### 分布式算法栈 (Distributed Algorithm Stack)
![](算法计算复杂度分析/image-93.webp)
...
upon round ∈ nacked do
    ...
    round := round + 1;
```
![](算法计算复杂度分析/image-78.webp)

##### 示例
- 无故障
    ![](算法计算复杂度分析/image-79.webp)
- 完成共识后故障，未触发换主
    ![](算法计算复杂度分析/image-80.webp)
- 完成共识后故障，触发换主，由于满足法定人数，可以通过 READ 读取潜在的共识值，最终仍能达成一致
    ![](算法计算复杂度分析/image-81.webp)
- 故障且未完成共识，触发换主，此时第一个提案被放弃，新的领导者提出自己的提案并达成一致
    ![](算法计算复杂度分析/image-82.webp)
...
- Properties:
    ...
    - GM2. **Uniform Agreement**: If some process installs a view $V = (id, M)$ and another process installs some view $V^\prime = (id, M^\prime)$, then $M = M^\prime$,（一致性：如果某个进程安装视图 $V = (id, M)$，另一个进程安装某个视图 $V^\prime = (id, M^\prime)$，则 $M = M^\prime$。）
...
### 示例
![](算法计算复杂度分析/image-89.webp)

### 正确性论证
- Monotonicity（单调性）
    - $id < id^\prime$
        - Assume $id^\prime \leqslant id$, i.e., consensus instance $id^\prime$ decides after $id$（即，共识实例 $id^\prime$ 在 $id$ 之后决定）
        - Impossible since a process will not propose to instance id, before all lower-numbered instances decide (wait flag)（不可能，因为进程不会在所有较低编号的实例决定之前向实例 id 提出新的提案（等待标志的约束））
    - $M^\prime \subsetneq M$
        - By uniform consensus Validity（通过统一共识的有效性）
        - Since a consensus is triggered only when correct $\subsetneq M$（因为仅当 correct $\subsetneq M$ 时才会触发共识）
- Agreement（一致性）
